...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        frame.size.width = 0.0; // should start from 0.0
        ...
        if ([UIApplication.sharedApplication.delegate.window respondsToSelector:@selector(setTintColor:)] &&
            UIApplication.sharedApplication.delegate.window.tintColor) {    // Have to make sure there is tintColor!!!
...
- (void)setProgress:(float)progress animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(isGrowing && animated) ? _barAnimationDuration : 0.0 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        frame.size.width = MAX(progress, 0.0) * self.bounds.size.width;
